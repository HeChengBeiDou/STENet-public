...
def save_batch_imgs(imgs, msks, msk_preds, save_path, test_data_name=None, acc=None):
    """
    Save a batch of images, masks, and predicted masks as individual images.

    Args:
        imgs (torch.Tensor): Batch of images with shape (n, c, h, w).
        msks (torch.Tensor): Batch of masks with shape (n, 1, h, w).
        msk_preds (torch.Tensor): Batch of predicted masks with shape (n, 1, h, w).
        save_path (str): Path to save the images.


        test_data_name (str, optional): Additional name for test data. Defaults to None.
    """
    n = imgs.shape[0]
    ...
    for i in range(n):
        img = imgs[i].permute(1, 2, 0).detach().cpu().numpy() # (h, w, c)
        img = img / 255. if img.max() > 1.1 else img

        msk = msks[i].permute(1, 2, 0).detach().cpu().numpy()
        msk_pred = msk_preds[i].permute(1, 2, 0).detach().cpu().numpy()

        plt.figure(figsize=(7, 15))

        plt.subplot(3, 1, 1)
        plt.imshow(img, cmap='gray')#如果不用灰度会导致颜色不对
        plt.axis('off')
        ...
        if acc is not None:
            acc_value = np.mean(acc[i][1:])
            filename = "{}/{}_{}.png".format(save_path, test_data_name[i], format(acc_value, ".4f"))
            plt.savefig(filename)
        else:
            filename = "{}/{}.png".format(save_path, test_data_name[i])
        plt.savefig(filename)
        plt.close()
...
class BceDiceLoss(nn.Module):
    ...
    def forward(self, pred, target_onehot):
        bceloss = self.bce(pred, target_onehot)
        diceloss = self.dice(pred, target_onehot)
...
class myResize:
    ...
    def __call__(self, data):
        ...
        return TF.resize(image, [self.size_h, self.size_w]), TF.resize(mask, [self.size_h, self.size_w], InterpolationMode.NEAREST)
...
def cal_params_flops(model, size, logger,device):
    input = torch.randn(2, 3, size, size).to(device=device)
