...
class DatasetCAMUS(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, img_size=224, use_original_imgsize=False, trn_image_only_operator=None, support_idx=None):
        ...
        support_idx = ["0451", "0452", "0453", "0454", "0455"]
        self.support_idx = support_idx## 451 452 453 454 455
        ...
        for i in range(len(self.img_file_list)):
            if curr_sec in self.img_file_list[i] and "NMLX" not in self.img_file_list[i]:
                img_file_list.append(self.img_file_list[i])
            if curr_sec in self.mask_file_list[i] and "NMLX" not in self.mask_file_list[i]:
    ...
    def __getitem__(self, idx):
        ...
        # curr_idx = (curr_query_idx + 1)%len(self.img_file_list)
        # self.support_idx
        # random.randint(0,len(self.img_file_list)-1)
        # while len(support_imgs) < self.shot:
        for i in range(self.shot):
            # print(curr_idx)
            curr_idx = self.support_idx[i]

            curr_support_img_file_name = "patient{}_{}_{}.png".format(curr_idx, curr_query_view, curr_query_time)
            curr_support_mask_file_name = "patient{}_{}_{}_gt.png".format(curr_idx, curr_query_view, curr_query_time)

            tmp_file_name,file_type = curr_support_img_file_name.split('.')
            patient, section, temporal = tmp_file_name.split('_')

            # if curr_query_patient not in support_patients and patient != curr_query_patient and curr_query_view == section and curr_query_time == temporal:
