...
class Visualizer:
    ...
    def initialize(cls, visualize):
        ...
        cls.colors = {'red': (255, 0, 0), 'blue': (0, 0, 255), 'green': (0, 255, 0)}
    ...
    @classmethod
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_idx,batch_name, iou_b=None):
        ...
        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id,curr_name) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, batch_name)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction(spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, True,curr_name, iou)
    ...
    @classmethod
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, label,batch_name, iou=None):
        #spt_imgs (shot,h,w,c)
        ...
        spt_masked_pils = [Image.fromarray(cls.apply_mask(spt_img, spt_mask, spt_color)) for spt_img, spt_mask in zip(spt_imgs, spt_masks)]#spt_img (h,w,c)
        ...
        iou = iou.item() if iou else 0.0#'%d_%d_class-%d_iou-%.4f'
        merged_pil.save(cls.vis_path + '%s_class-%d_iou-%.4f' % (batch_name, cls_id, iou) + '.jpg')
